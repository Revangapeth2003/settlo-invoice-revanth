...
router.patch('/:invoiceNumber/mark-paid', async (req, res) => {
  try {
    console.log('Marking invoice as paid:', req.params.invoiceNumber);

    const invoice = await Invoice.findOne({ invoiceNumber: req.params.invoiceNumber });
    if (!invoice) {
      console.log('Invoice not found:', req.params.invoiceNumber);
      return res.status(404).json({ success: false, message: 'Invoice not found' });
    }

    // Set status and payment fields explicitly
    invoice.status = 'Paid';
